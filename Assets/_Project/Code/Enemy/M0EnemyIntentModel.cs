...
namespace GlassRefrain.Enemy
{
    public sealed class M0EnemyIntentModel
    {
        ...
        public M0EnemyIntentModel(string enemyId = "M0Enemy")
        {
            ...
            attackIntent = new EnemyAttackIntentContext(string.Empty, string.Empty, 0f, new EnemyAttackTagSet(Array.Empty<string>()));
            punishWindow = new EnemyPunishWindowContext(false, 0f, string.Empty);
            RefreshSnapshot();
        }

        public EnemyIntentSnapshot Snapshot
        {
        ...
        public void EnterRecovery(float durationSeconds, string reason, bool openPunishWindow, float punishWindowSeconds, string punishSource)
        {
        ...
        {
            string source = string.IsNullOrEmpty(reason) ? punishWindow.Source : reason;
            punishWindow = new EnemyPunishWindowContext(false, 0f, source ?? string.Empty);
        }

        public void Tick(float deltaSeconds)
        {
            float nextRemaining = remainingSeconds - deltaSeconds;
            remainingSeconds = nextRemaining > 0f ? nextRemaining : 0f;

            if (telegraph.IsActive)
            {
                float telegraphRemaining = telegraph.RemainingSeconds - deltaSeconds;
            ...
            {
                float punishRemaining = punishWindow.RemainingSeconds - deltaSeconds;
        ...
        private void RefreshSnapshot()
        {
            ...
            Action<EnemyIntentSnapshot> handler = SnapshotChanged;
            if (handler != null)
            {
