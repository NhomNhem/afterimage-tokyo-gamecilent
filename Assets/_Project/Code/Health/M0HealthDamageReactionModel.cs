...
namespace GlassRefrain.Health
{
    public sealed class M0HealthDamageReactionModel
    {
        ...
        public HealthStateSnapshot Snapshot
        {
        ...
        public DamageApplicationResult ApplyDamage(DamageApplicationContext request)
        {
            ...
            {
                lastDamageResult = new DamageApplicationResult(DamageApplicationResultType.Ignored, "Target is already disabled", 0f);
                RefreshSnapshot();
            ...
            {
                lastDamageResult = new DamageApplicationResult(DamageApplicationResultType.Rejected, "Damage amount must be greater than zero", 0f);
                RefreshSnapshot();
                return lastDamageResult;
            }

            float applied = request.Amount;
            ...
            lastDamageResult = new DamageApplicationResult(DamageApplicationResultType.Accepted, "Damage applied", applied);
            RefreshSnapshot();
            return lastDamageResult;
        }
        ...
        public void EnterRecovery(string reason, float suppressionSeconds)
        {
            ...
            hitReaction = new HitReactionContext(hitReaction.SourceId, string.IsNullOrEmpty(reason) ? "Recovering" : reason, suppressionSeconds);
            RefreshSnapshot();
        }
        ...
        public void EnterLiving(string reason)
        {
            ...
            state = HealthState.Living;
            hitReaction = new HitReactionContext(hitReaction.SourceId, string.IsNullOrEmpty(reason) ? "Living" : reason, 0f);
            RefreshSnapshot();
        }
        ...
        private void RefreshSnapshot()
        {
            ...
            Action<HealthStateSnapshot> handler = SnapshotChanged;
            if (handler != null)
            {
