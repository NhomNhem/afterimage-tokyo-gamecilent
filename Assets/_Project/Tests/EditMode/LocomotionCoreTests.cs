...
namespace GlassRefrain.Tests.EditMode {
    public class LocomotionCoreTests {
        [Test]
        ...
        public void LocomotionFilesDoNotReferenceLegacyInputManagerOrGeneratedDi() {
            string[] files = {
                ...
                "Assets/_Project/Code/Locomotion/LocomotionCore.cs",
        ...
        public void DodgeDisplacementMovesPlayerWhenTriggered() {
            var locomotion = new LocomotionCore(new M0LocomotionSettings(
                moveSpeed: 5f,
                inputDeadzone: 0.1f,
                facingLerpSpeed: 8f,
                dodgeDistance: 1.5f,
            ...
            var inputIntent = CreateInputSnapshot(1f, 0f, true);
            locomotion.ConsumeInputIntent(inputIntent);
            locomotion.Tick(inputIntent, new ControlState(true, true, true), 0.016f);
            var before = locomotion.GetMovementSnapshot().Position;

            Assert.That(locomotion.TryBeginDodgeDisplacement(), Is.True);
            locomotion.Tick(inputIntent, new ControlState(true, true, true), 0.1f);
        ...
        public void InvalidDodgeSettingsFailFast() {
            Assert.Throws<ArgumentOutOfRangeException>(() => new LocomotionCore(
        ...
        public void ResetForEncounter_UsesProvidedAuthoredBaselineEvenWhenNonZero() {
            ...
            var inputIntent = CreateInputSnapshot(-1f, 0f, true);
            locomotion.ConsumeInputIntent(inputIntent);
            locomotion.Tick(inputIntent, new ControlState(true, true, true), 0.2f);
